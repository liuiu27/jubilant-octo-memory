...
public class IhuyiUtils {
    ...
    public static IhuyiRechargeRes ihuyiPhoneRecharge(RechargeOrderVo orderVo, RechargeReq rechargeReq){
        ...
        //step4.获取签名数据
        String sign = IhuyiUtils.getSign(map);
     ...
     * @return
     */
    public static IhuyiRechargeRes ihuyiTrafficRecharge(RechargeOrderVo orderVo, RechargeReq rechargeReq){
        log.info("互亿流量充值工具类...");
        IhuyiRechargeRes rechargeRes = new IhuyiRechargeRes();
        String timestamp = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
        //step1.获取互亿流量充值url
        ...
        String sign = IhuyiUtils.getSign(map);
        String url = domain+"?action="+action+"&username="+username+"&mobile="+mobile+"&orderid="+orderid+"&package="+packag+"&timestamp="+timestamp+"&sign="+sign;
        //String phoneUrl= "http://f.ihuyi.com/phone?action="+action+"&username="+username+"&mobile="+mobile+"&orderid="+orderid+"&package="+packag+"&timestamp="+timestamp+"&sign="+sign;
        try {
            log.info("互亿流量充值订购，get请求url为："+url);
            //https://api.ihuyi.com/f/v2?action=recharge&username=cf_testapi&mobile=15737317830&orderid=18030817213564050309&package=20&timestamp=20180308172322&sign=0e466a9c19e6b0e6b9ce9df438f9c531
            String str = HttpUtil.doGet(url);
            if (!StringUtils.isEmpty(str)) {
                rechargeRes = JSON.parseObject(str, IhuyiRechargeRes.class);
    ...
    public static IhuyiRechargeRes ihuyiVirtualGoodsRechargeBuy(String orderid,String account,String productid,Long rechargeNumber,String extend,String buyerip){
        log.info("互亿虚拟充值工具类...");
        IhuyiRechargeRes rechargeRes = new IhuyiRechargeRes();
        //step1.获取互亿虚拟充值url
        String domain = null;
     ...
     * @return
     */
    public static IhuyiVoucherRes ihuyiGiftCardBuy(VoucherOrderVo orderVo, String productid , GetVoucherReq voucherReq){
        log.info("互亿虚拟充值工具类...");
     ...
     * @return
     */
    public static IhuyiOrderQueryRes ihuyiRechargeQuery(ServiceOrder serviceOrder){
     ...
     * @return
     */
    public static IhuyiVirtualOrderQueryRes virtualGoodsRechargeQuery(ServiceOrder serviceOrder){
